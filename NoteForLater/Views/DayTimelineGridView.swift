...
struct DayTimelineGridView: View {
            ...
            VStack(spacing: 0) {
                ...
                if occurrenceLists.isSplitAtNoon {
                    DayTimelineSegment(
                        ...
                        habitStreaks: cachedHabitStreaks,
                        precedingContentHeight: DayTimelineGeometry.precedingContentHeight(
                            twoMinuteSectionHeight: twoMinuteSectionHeight, amSectionHeight: amSectionHeight
                        ),
                    ...
                    DayTimelineSegment(
                        ...
                        habitStreaks: cachedHabitStreaks,
                        precedingContentHeight: DayTimelineGeometry.afternoonPrecedingContentHeight(
                            twoMinuteSectionHeight: twoMinuteSectionHeight, amSectionHeight: amSectionHeight,
                            morningDayHeight: dayHeight(for: morningQuarterRange), middaySectionHeight: middaySectionHeight
                        ),
                        scrollPosition: $scrollPosition,
                        scrollGeometry: scrollGeometry,
    ...
    private func scrollToRoughlyNow() {
        ...
        scrollPosition.scrollTo(y: DayTimelineGeometry.scrollToRoughlyNowOffset(
            twoMinuteSectionHeight: twoMinuteSectionHeight,
            amSectionHeight: amSectionHeight,
            isSplitAtNoon: isSplitAtNoon,
            targetQuarter: targetQuarter,
            middaySplitQuarter: middaySplitQuarter,
            morningDayHeight: dayHeight(for: morningRange),
            middaySectionHeight: middaySectionHeight,
            afternoonRangeStart: afternoonRange.start,
    ...
    private func twoMinuteRowView(_ row: TaskItem.TwoMinuteRow) -> some View {
        ...
        case .task(let task):
            ...
            .rowCardLongPress(onTap: {
                ...
                // safe.
                // Same `ChooseDayPlanning.pushDay` every other surface uses — the
                ...
                TwoMinutePush.cycle(
                    task,
                    planDate: ChooseDayPlanning.pushDay(missedOn: targetDate, calendar: Calendar.current),
    ...
    private func cycleRecurringTaskOccurrence(task: TaskItem) {
        ...
        let outcome = ScheduleReviewViewModel.cycleRecurringOccurrenceReconcilingPush(
            task: task,
            on: today,
            plannedDay: ChooseDayPlanning.pushDay(missedOn: today, calendar: calendar),
            context: modelContext,
            calendar: calendar
        )
        habitOccurrenceRefreshTick += 1
